...
if(!isset($_SESSION['is_login'])){
  if(isset($_REQUEST['rEmail'])){
    ...
    $sql = "SELECT * FROM requesterlogin_tb WHERE r_email='".$rEmail."' AND r_password='".$rPassword."' limit 1";
    $result = $conn->query($sql);
    if($result->num_rows == 1){
      $_SESSION['is_login'] = true;
      $_SESSION['rEmail'] = $rEmail;

      // PURPLE: Get requester stored in database to session
      $_SESSION['requester'] = $result->fetch_assoc();
