...
if(isset($_SESSION['is_adminlogin'])){
 ...
 if(isset($_REQUEST['view'])){
  $sql = "SELECT * FROM submitrequest_tb LEFT JOIN requestinfo_tb ON submitrequest_tb.request_info=requestinfo_tb.ri_id WHERE request_id = {$_REQUEST['id']}";
 ...
 if(isset($_REQUEST['assign'])){
  ...
  if(
   ...
   $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert"> Fill All Fileds </div>';
    ...
    $requesterid = $_REQUEST['requester_id'];
    ...
    $sql = "INSERT INTO assignwork_tb (request_id, request_info, request_desc, request_quantity, requester_name, requester_add1, requester_add2, requester_city, requester_state, requester_zip, requester_email, requester_mobile, assign_tech, assign_date, requester_id) VALUES ('$rid', '$rinfo','$rdesc', '$rquantity', '$rname', '$radd1', '$radd2', '$rcity', '$rstate', '$rzip', '$remail', '$rmobile', '$rassigntech', '$rdate', '$requesterid')";
...
<div class="col-sm-5 mt-5 jumbotron">
  ...
  <form action="" method="POST">
    <input type="hidden" name="requester_id" value="<?php if(isset($row['requester_id'])) {echo $row['requester_id']; }?>" readonly>
    ...
    <div class="form-group">
      ...
      <input type="number" min="1" class="form-control" id="requestdesc" name="requestquantity" value="<?php if(isset($row['request_quantity'])) { echo $row['request_quantity']; } ?>">
    </div>
    <div class="form-group">
