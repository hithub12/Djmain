...
<script>
  ...
  const ELEMENTS = {
    ...
    QUANTITY: document.querySelector("#pquantity"),
    PRICEEACH: document.querySelector("#psellingcost"),
  }

  const handleElementInput = (event) => {
    const quantity = ELEMENTS.QUANTITY.value;
    const priceeach = ELEMENTS.PRICEEACH.value;
    if (quantity.length > 0 && priceeach.length > 0) {
      ELEMENTS.TOTALCOST.value = Number.parseInt(quantity * priceeach);
    }
  }

  ELEMENTS.QUANTITY.addEventListener("input", handleElementInput);
  ELEMENTS.PRICEEACH.addEventListener("input", handleElementInput);
