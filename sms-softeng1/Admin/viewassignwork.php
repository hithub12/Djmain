...
<div class="col-sm-6 mt-5  mx-3">
 ...
 if(isset($_REQUEST['view'])){
  $sql = "SELECT * FROM assignwork_tb LEFT JOIN requestinfo_tb ON assignwork_tb.request_info=requestinfo_tb.ri_id WHERE request_id = {$_REQUEST['id']}";
 ...
 <table class="table table-bordered">
  <tbody>
   ...
   <tr>
    <td>Request Info</td>
    <td>
     <?php if(isset($row['request_info'])) {echo $row['ri_details'] . ' <strong>(₱' . $row['ri_price'] . ')</strong>'; }?>
    </td>
   </tr>
   <tr>
    <td>Quantity</td>
    <td>
     <?php if(isset($row['request_quantity'])) { echo $row['request_quantity']; }?>
    </td>
   </tr>
   <tr>
